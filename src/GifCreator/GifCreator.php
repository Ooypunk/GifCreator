...
class GifCreator {
	...
	public function create($frames = array(), $durations = array(), $loop = 0) {
		...
		for ($i = 0; $i < count($frames); $i++) {
			if (is_resource($frames[$i])) { // Resource var
				...
				$frames[$i] = $this->getContentsIfFile($frames[$i]);
			...
			if (!$this->stringHasValidGifHeader($this->frame_sources[$i])) {
	...
	public function stringHasValidGifHeader($string) {
		$is_87a = substr($string, 0, 6) === 'GIF87a';
		$is_89a = substr($string, 0, 6) === 'GIF89a';
		return $is_87a || $is_89a;
	}

	public function getContentsIfFile($string) {
		if (!is_string($string)) {
			throw new Exception('getContentsIfFile expects string as argument');
		}
		if ($this->stringHasValidGifHeader($string)) {
			return $string;
		}
		if (file_exists($string) || filter_var($string, FILTER_VALIDATE_URL)) {
	...
	public function addGifFrames($i, $d) {
		...
		if (ord($this->frame_sources[$i] { 10 }) & 0x80 && $this->img_was_built) {
	...
	/**
	 * Compare two blocks and return the version
	 * 
	...
	public function reset() {
		$this->frame_sources = null;
		$this->gif = 'GIF89a'; // the GIF header
		$this->img_was_built = false;
		$this->loop = 0;
		$this->dis = 2;
		$this->colour = -1;
	}
